...
function sendMail(email, code) {
    const data = JSON.stringify({ to: email, subject: 'DormLift Code', html: `Your code: <b>${code}</b>` });
...
app.post('/api/auth/register', async (req, res) => {
    ...
    db.get(`SELECT * FROM verify_codes WHERE email = ?`, [email], async (err, row) => {
        if (!row || row.code !== code) return res.status(400).json({ success: false, message: 'Code error' });
...
app.listen(PORT, '0.0.0.0', () => console.log(`🚀 Production server logic running on port ${PORT}`));
