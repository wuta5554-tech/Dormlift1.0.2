...
let db = new sqlite3.Database(DB_PATH, (err) => {
    ...
    db.exec(`
        ...
        CREATE TABLE IF NOT EXISTS verify_codes (email TEXT PRIMARY KEY, code TEXT, expire_at INTEGER);
        CREATE TABLE IF NOT EXISTS users (
            id INTEGER PRIMARY KEY AUTOINCREMENT, student_id TEXT UNIQUE, school_name TEXT,
            first_name TEXT, given_name TEXT, gender TEXT, anonymous_name TEXT,
            phone TEXT UNIQUE, email TEXT UNIQUE, password TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP
        );
        CREATE TABLE IF NOT EXISTS tasks (
            id INTEGER PRIMARY KEY AUTOINCREMENT, publisher_id TEXT, move_date TEXT, move_time TEXT,
            from_address TEXT, to_address TEXT, items_desc TEXT, people_needed INTEGER, 
            reward TEXT, status TEXT DEFAULT 'pending', helper_id TEXT
        );
    `);
});
...
function sendMail(email, code) {
    const data = JSON.stringify({ to: email, subject: 'DormLift Code', html: `Your code is: <b>${code}</b>` });
...
app.post('/api/auth/send-code', (req, res) => {
    ...
    db.run(`INSERT OR REPLACE INTO verify_codes VALUES (?, ?, ?)`, [req.body.email, code, expire], () => {
...
app.post('/api/auth/register', async (req, res) => {
    ...
    db.get(`SELECT * FROM verify_codes WHERE email = ?`, [email], async (err, row) => {
        if (!row || row.code !== code || Date.now() > row.expire_at) return res.status(400).json({ success: false, message: 'Invalid or expired code' });
        ...
        db.run(`INSERT INTO users (student_id, school_name, first_name, given_name, gender, anonymous_name, phone, email, password) VALUES (?,?,?,?,?,?,?,?,?)`,
            [student_id, school_name, first_name, given_name, gender, anonymous_name, phone, email, hashed], (err) => {
                if (err) return res.status(400).json({ success: false, message: 'Student ID or Phone already exists' });
...
app.post('/api/auth/login', (req, res) => {
    db.get(`SELECT * FROM users WHERE student_id = ?`, [req.body.student_id], async (err, user) => {
        if (!user || !(await bcrypt.compare(req.body.password, user.password))) return res.status(400).json({ success: false, message: 'Invalid ID or Password' });
...
app.post('/api/task/my-published', (req, res) => {
    db.all(`SELECT * FROM tasks WHERE publisher_id = ?`, [req.body.student_id], (err, rows) => res.json({ success: true, list: rows }));
});

app.post('/api/task/my-assigned', (req, res) => {
    db.all(`SELECT * FROM tasks WHERE helper_id = ?`, [req.body.student_id], (err, rows) => res.json({ success: true, list: rows }));
});
